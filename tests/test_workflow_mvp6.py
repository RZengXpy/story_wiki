...
def test_mvp6_workflow_with_scripts():
    ...
    for scene_id, script_node in result.graph.scripts.items():
        ...
        assert len(script_node.content) > 0, f"Script {scene_id} has empty content"
    ...
    # ── MVP 6.3: Merger report includes script stats ──────────────────────
    ...
    assert report["scripts_generated"] == scripts_count
    print(f"  [OK] Merger report: {report['scripts_generated']} scripts, {report['screenplay_items']} items")

    # ── MVP 6.4: to_yaml includes scripts ─────────────────────────────────
    ...
    # ── MVP 6.5: summary() includes script count ──────────────────────────
